...
- **Prompt cache injection on anthropic passthrough**: In the Anthropic-native passthrough path (`ApiFormat="anthropic"`), if the request body contains no `cache_control` anywhere (checked via substring match on `"cache_control"`), a top-level `cache_control: {"type":"ephemeral"}` is injected so anthropic-compatible upstreams (e.g. MiniMax, opencode.ai) enable prompt caching automatically. Client-supplied `cache_control` (block-level or top-level) is always forwarded untouched — including in the OpenAI-format conversion path, which no longer strips it.
- **Strip gate for non-Claude models**: `LlmService:StripNonClaudeModels` (appsettings, default `false`) gates the ENTIRE request preprocessing in the OpenAI-format path (`ApiFormat="openai"`). Off (default): the request body is forwarded byte-for-byte to `/v1/chat/completions` — no format conversion, no model rewrite, no field dropping, no system-reminder/noise filtering, no tool filtering, no Qwen system prompt replacement, no `stream`/`tool_choice` forcing. On: the full Anthropic→OpenAI conversion + slimming pipeline runs as before. Toggleable at runtime via `POST /override-model` (`StripNonClaudeModels`). Note: the Qwen response handler expects the converted (non-stream JSON) flow, so the Qwen path effectively requires the setting to be on. The anthropic passthrough path is unaffected by this setting.
